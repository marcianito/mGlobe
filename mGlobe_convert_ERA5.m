function mGlobe_convert_ERA5(start_calc,end_calc,time_resol,file,ghc_path)
...
try
    ...
    set(findobj('Tag','text_status'),'String','Models: Load valid ERA5  (netCDF) file'); drawnow 
    return
end
...
for i = 1:size(time,1);                                                     % create new file for each time epoch
    ...
    save(nazov,'out_mat');                                                  % save create matrix
        ...
        out_mat = [];
        if time_resol == 6
            out_message = sprintf('Models: Warning: data for ERA5_M_%4d%02d.mat not found!',time(i,1),time(i,2)); % create warning message
        else
            out_message = sprintf('Models: Warning: data for ERA5_6H_%4d%02d%02d_%02d.mat not found!',time(i,1),time(i,2),time(i,3),time(i,4));
        end
