function mGlobe_convert_LSDM(start_calc,end_calc,time_resol,file,ghc_path)
...
for i = 1:size(time,1);                                                     % create new file for each time epoch
    ...
    if size(time,1) > 2
        ...
        out_message = sprintf('Models: converting LSDM model ...'); % create status message
    end
    ...
    if time_resol == 6                                                      % create new output file name (monthly or hourly data)
            nazov = fullfile(ghc_path,sprintf('LSDM_M_%4d%02d.mat',time(i,1),time(i,2))); 
    else
            nazov = fullfile(ghc_path,sprintf('LSDM_6H_%4d%02d%02d_%02d.mat',time(i,1),time(i,2),time(i,3),time(i,4)));
    end
    save(nazov,'out_mat');                                                  % save create matrix
    else
        out_mat = [];
        if time_resol == 6
            out_message = sprintf('Models: Warning: data for LSDM_M_%4d%02d.mat not found!',time(i,1),time(i,2)); % create warning message
        else
            out_message = sprintf('Models: Warning: data for LSDM_6H_%4d%02d%02d_%02d.mat not found!',time(i,1),time(i,2),time(i,3),time(i,4));
        end
